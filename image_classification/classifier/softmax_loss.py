...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in xrange(num_train):
        for j in xrange(num_class):
            loss += -(y_trueClass[i, j] * np.log(prob[i, j]))    # L = -(1/N)∑i∑j1(k=yi)log(exp(fk)/∑j exp(fj)) + λR(W)
            dW_each[:, j] = -(y_trueClass[i, j] - prob[i, j]) * X[i, :] #  ∇Wk L = -(1/N)∑i xiT(pi,m-Pm) + 2λWk, where Pk = exp(fk)/∑j exp(fj
        dW += dW_each
    loss /= num_train
